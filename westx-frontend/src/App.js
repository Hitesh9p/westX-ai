...
function App() {
  ...
  const updateAnalytics = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/tweets/analytics');
  ...
  const handleScheduleTweet = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/tweets/schedule', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleGenerateTweet = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/tweets/generate', {
        method: 'POST'
      });
      const data = await response.json();
